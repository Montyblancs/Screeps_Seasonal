var creep_baseOp = {
    ...
    run: function(creep) {
        ...
        if (creep.ticksToLive <= 250 && Memory.powerSpawnList[creep.room.name]) {
            ...
            let pSpawn = Game.getObjectById(Memory.powerSpawnList[creep.room.name][0]);
            if (pSpawn) {
                if (!creep.carry[RESOURCE_POWER]) {
                    let powerToTake = 100;
                    if (pSpawn.store[RESOURCE_POWER]) {
                        powerToTake = 100 - pSpawn.store[RESOURCE_POWER];
                    }
                    let withdrawResult = creep.withdraw(creep.room.storage, RESOURCE_POWER, powerToTake);
                    if (withdrawResult == ERR_NOT_IN_RANGE) {
                        creep.travelTo(creep.room.storage, {
                            ignoreRoads: true,
                            maxRooms: 1
