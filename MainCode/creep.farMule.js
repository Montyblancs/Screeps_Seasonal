let creep_farMule = {
    run: function(creep, doExcessWork) {
        ...
        if (creep.store[RESOURCE_ENERGY] > 0 && doExcessWork) {
            ...
            if (!creep.memory.storing) {
                ...
                if (creep.room.name != creep.memory.destination && creep.memory.containerPosition) {
                    ...
                    if (Game.flags[creep.room.name + "SKRoom"] && !foundScore) {
                        let droppedResources = creep.pos.findInRange(FIND_DROPPED_RESOURCES, 2)
                        if (droppedResources.length && creep.pickup(droppedResources[0]) == ERR_NOT_IN_RANGE) {
