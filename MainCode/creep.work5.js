    ...
    run: function(creep) {
        ...
        switch (creep.memory.priority) {
            ...
            case 'muleNearDeath':
                ...
                if (_.sum(creep.carry) <= 15) {
                    ...
                    if (linkTarget && linkTarget.energy >= 600) {
                        ...
                        } else if (creep.room.terminal && storageTarget.store[RESOURCE_ENERGY] < 155000 && creep.room.terminal.store[RESOURCE_ENERGY] > 31000) {
                            storageTarget = creep.room.terminal;
                        }
                        if (storageTarget) {
                            ...
                            if (targets) {
                                ...
                                if (terminalTarget) {
                                    ...
                                    if (creep.room.storage) {
                                        if (creep.room.storage.store[RESOURCE_ENERGY] >= 155000) {
                                            targetEnergy = 60000;
                                        } else if (creep.room.storage.store[RESOURCE_ENERGY] >= 150000) {
                                            targetEnergy = 30000;
                                        }
                                    }
            ...
            case 'distributorNearDeath':
                ...
                if (_.sum(creep.carry) <= 0) {
                    ...
                    if (!locatedTarget && Memory.roomsUnderAttack.indexOf(creep.room.name) === -1) {
                        let groundItems = creep.pos.findInRange(FIND_DROPPED_RESOURCES, 10, {
                    		filter: (thisResource) => (thisResource.resourceType == RESOURCE_ENERGY && thisResource.amount >= creep.store.getFreeCapacity())
                		});
                ...
                } else if (Memory.roomsUnderAttack.indexOf(creep.room.name) == -1 && creep.room.terminal && creep.room.storage && creep.room.storage.store[RESOURCE_ENERGY] < 150000 && creep.room.terminal.store[RESOURCE_ENERGY] > 31000) {
