var market_buyers = {
    ...
    run: function(thisRoom, thisTerminal, thisMineral) {
        ...
        if (TerminalEnergy >= 5000) {
            ...
            if (!hasSent && TerminalEnergy >= 50000 && Memory.energyNeedRooms.length && Memory.energyNeedRooms[0] != thisRoom.name && thisRoom.storage && thisRoom.storage.store[RESOURCE_ENERGY] >= 155000) {
...
function sendMineral(thisMineral, thisTerminal, targetRoom, saveFlag, nukerLimit) {
	if (targetRoom == thisTerminal.room.name) {
		//nani the fuck
		return false;
	}
