...
module.exports.loop = function() {
                ...
                if (splitList.length > 1) {
                    ...
                    if (Game.time >= parseInt(timeToCheck)) {
                        try {
                            ...
                            if (splitList[0].includes("FarMining") && Memory.FarRoomsUnderAttack.indexOf(Game.flags[TF].pos.roomName) == -1) {
    ...
    for (const i in Game.spawns) {
        ...
        if (thisRoom.controller.owner) {
            ...
            if (Memory.RoomsRun.indexOf(thisRoom.name) < 0) {
                ...
                if (Game.time % 100 == 0 || !Memory.repairTarget[thisRoom.name]) {
                ...
                if ((Game.time + 1) % 10000 == 0) {
                    ...
                    if (!Game.flags["DoNotClear"] && _.size(Game.constructionSites) >= MAX_CONSTRUCTION_SITES) {
                        for (var s in Game.constructionSites) {
                            if (Game.constructionSites[s].structureType == STRUCTURE_ROAD) {
                                Game.constructionSites[s].remove();
                            }
                ...
                if (Game.time % 10000 == 0) {
                    ...
                    thisRoom.createConstructionSite(thisRoom.controller.pos.x-1, thisRoom.controller.pos.y, STRUCTURE_RAMPART);
                    thisRoom.createConstructionSite(thisRoom.controller.pos.x-1, thisRoom.controller.pos.y-1, STRUCTURE_RAMPART);
                    thisRoom.createConstructionSite(thisRoom.controller.pos.x, thisRoom.controller.pos.y-1, STRUCTURE_RAMPART);
                    thisRoom.createConstructionSite(thisRoom.controller.pos.x+1, thisRoom.controller.pos.y-1, STRUCTURE_RAMPART);
                    thisRoom.createConstructionSite(thisRoom.controller.pos.x+1, thisRoom.controller.pos.y, STRUCTURE_RAMPART);
                    thisRoom.createConstructionSite(thisRoom.controller.pos.x+1, thisRoom.controller.pos.y+1, STRUCTURE_RAMPART);
                    thisRoom.createConstructionSite(thisRoom.controller.pos.x, thisRoom.controller.pos.y+1, STRUCTURE_RAMPART);
                ...
                if (Game.time % 50 == 0 && thisRoom.terminal && thisRoom.storage) {
                    if (Memory.energyNeedRooms.indexOf(thisRoom.name) === -1 && thisRoom.storage.store[RESOURCE_ENERGY] < 150000 && thisRoom.terminal.store[RESOURCE_ENERGY] < 50000) {
                    ...
                    } else if (Memory.energyNeedRooms.indexOf(thisRoom.name) != -1 && (thisRoom.storage.store[RESOURCE_ENERGY] >= 155000 || thisRoom.terminal.store[RESOURCE_ENERGY] >= 50000)) {
                ...
                if (Game.time % 50 == 0 && thisRoom.terminal) {
                    ...
                    let roomMinerals = _.keys(thisRoom.terminal.store);
                    ...
                    if (Game.rooms[Memory.observationPointers[thisRoom.name][2]]) {
                        ...
                        /*if (!Game.flags[thisRoom.name + "PowerGather"] && thisRoom.storage && (!thisRoom.storage.store[RESOURCE_POWER] || thisRoom.storage.store[RESOURCE_POWER] < 50000) && thisRoom.storage.store[RESOURCE_ENERGY] >= 400000) {
                            ...
                            if (powerbanks.length) {
                                ...
                                console.log('Observed and created power flag at ' + Memory.observationPointers[thisRoom.name][2])
                            }
                        } else if (Game.flags[thisRoom.name + "PowerGather"] && Game.rooms[Game.flags[thisRoom.name + "PowerGather"].pos.roomName]) {
                            let powerbanks = Game.rooms[Game.flags[thisRoom.name + "PowerGather"].pos.roomName].find(FIND_STRUCTURES, {
                ...
                //observationPointers
                //[x Offset, y Offset, Observed Room Name]
            ...
            if (Game.time % delay == 0 && Memory.NoSpawnNeeded.indexOf(thisRoom.name) < 0 && Game.spawns[i].isActive() && !Game.spawns[i].spawning) {
                ...
                if (!Memory.isSpawning && (thisRoom.storage && thisRoom.storage.store[RESOURCE_ENERGY] <= 900000 || thisRoom.controller.level < 8) && Game.cpu.bucket >= 1000 && Memory.roomsUnderAttack.indexOf(thisRoom.name) == -1) {
    ...
    if (Memory.postObserveTick && Game.time % 5 != 0) {
...
function DisplayBoostTotals() {
    ...
    if (Memory.warMode) {
        new RoomVisual().rect(6.5, 44.5, 7.1, 1, {
            fill: fillColor,
            stroke: '#FFFFFF',
            opacity: 0.15,
            strokeWidth: 0.15
        });
        new RoomVisual().text("WAR MODE ENABLED", 6.7, 45.2, {
            align: 'left',
            font: '0.7 Courier New',
            color: '#FFFFFF',
            stroke: '#000000',
    ...
    new RoomVisual().text("SCORE : " + formatNumber(Memory.mineralTotals[RESOURCE_SCORE]), 6.7, 46.7, {
        align: 'left',
        font: '0.7 Courier New',
        color: '#FFFFFF',
        stroke: '#000000',
