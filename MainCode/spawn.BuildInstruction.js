    ...
    run: function(spawn, instruction, params, energyIndex, thisRoom = '', params2 = '') {
        switch (instruction) {
            ...
            case 'assault':
                ...
                if (Game.flags[spawn.room.name + "WallBreaker"] && attackers.length < 1) {
                    //Seasonal -
                    //Spam out an attacker to crack open the score collector
                    let priorityName = 'assattacker';
                    let attackerConfig = getWallCrackerBuild(spawn.room.energyCapacityAvailable);
                ...
                } else if (attackers.length < 2 && (attackers.length < healers.length || attackers.length == healers.length)) {
                    let priorityName = 'assattacker';
            ...
            case 'scoreRunner':
                ...
                if (runners.length < runnerMax) {
                    ...
                    if (configCost <= Memory.CurrentRoomEnergy[energyIndex]) {
                        Memory.CurrentRoomEnergy[energyIndex] = Memory.CurrentRoomEnergy[energyIndex] - configCost;
                        spawn.spawnCreep(mConfig, 'IgnoreMe_' + spawn.name + '_' + Game.time, {
                            memory: {
                ...
                break;
            case 'scoreBlocker':
                var blockers = _.filter(Game.creeps, (creep) => (creep.memory.priority == 'scoreBlocker' && creep.memory.homeRoom == spawn.room.name));
                if (blockers.length < 1) {
                    let mConfig = [MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,ATTACK,HEAL,HEAL,HEAL,HEAL,HEAL]
                    let configCost = calculateConfigCost(mConfig);
                    if (configCost <= Memory.CurrentRoomEnergy[energyIndex]) {
                        Memory.CurrentRoomEnergy[energyIndex] = Memory.CurrentRoomEnergy[energyIndex] - configCost;
                        spawn.spawnCreep(mConfig, 'BegoneThots_' + spawn.name + '_' + Game.time, {
                            memory: {
                                priority: 'scoreBlocker',
                                homeRoom: spawn.room.name,
                                destination: params,
                                deathWarn: _.size(mConfig) * 4
                            }
...
function getWallCrackerBuild(energyCap) {
    let thisConfig = [];

    let ConfigCost = BODYPART_COST[MOVE] + BODYPART_COST[WORK] + BODYPART_COST[WORK];

    while ((energyCap / ConfigCost) >= 1) {
        thisConfig.push(MOVE);
        if (thisConfig.length >= 50) {
            energyCap = energyCap - (ConfigCost - BODYPART_COST[MOVE]);
            break;
        }
        thisConfig.push(WORK);
        if (thisConfig.length >= 50) {
            energyCap = energyCap - (ConfigCost - (BODYPART_COST[MOVE] + BODYPART_COST[WORK]));
            break;
        }
        thisConfig.push(WORK);
        if (thisConfig.length >= 50) {
